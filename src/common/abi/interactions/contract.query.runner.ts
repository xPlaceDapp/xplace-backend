import { type ContractQueryResponse } from '@multiversx/sdk-network-providers'
import { type INetworkProvider } from '@multiversx/sdk-network-providers/out/interface'
import { ResultsParser, type SmartContract, type Interaction, type TypedOutcomeBundle } from '@multiversx/sdk-core'
...
export class ContractQueryRunner {
  ...
  async runQuery(contract: SmartContract, interaction: Interaction): Promise<TypedOutcomeBundle> {
    try {
      ...
      this.logger.log(`Unexpected error when running query '${interaction.buildQuery().func.toString()}' to sc '${contract.getAddress().bech32()}' `)
